...
#import "AFHTTPClient+DataSync.h"

#import "DataSyncObject.h"
#import "DataSync+Internal.h"
#import "DataSyncSignIn+Internal.h"
#import "DataSyncError.h"


#define FAIL_AND_RETURN_WITH_CODE(errorcode) \
    FAIL_AND_RETURN([NSError errorWithDomain:kDataSyncServicesErrorDomain code:(errorcode) userInfo:nil])
...
{
    DataSyncObject *instance = [[self alloc] initWithClassName:className];
...
- (void)saveOnSuccess:(void (^)(DataSyncObject *object))success
...
{
    return [NSError errorWithDomain:kDataSyncServicesErrorDomain code:DataSyncServicesAuthorizationRequired userInfo:nil];
}
...
- (void)fetchOnSuccess:(void (^)(DataSyncObject *object))success
               failure:(void (^)(NSError *error))failure
{
...
- (void)deleteOnSuccess:(void (^)(DataSyncObject *object))success
                failure:(void (^)(NSError *error))failure
{
...
- (void)performMethod:(NSString *)method
           parameters:(NSDictionary *)parameters
            onSuccess:(void (^)(DataSyncObject *object))success
              failure:(void (^)(NSError *error))failure
{
              ...
              failure:(void (^)(NSError *error))failure
{
    if (!self.objectID || self.objectID.length <= 0) {
        FAIL_AND_RETURN_WITH_CODE(DataSyncServicesObjectIDRequired);
    }
    
    if (attempts <= 0) {
        FAIL_AND_RETURN_WITH_CODE(DataSyncServicesAuthorizationRequired);
    }
    
    NSError *error;
    AFHTTPClient *client = [[DataSyncSignIn sharedInstance] dataServiceClient:&error];
...
- (HTTPSuccessBlock)successBlockForMethod:(NSString *)method
                                  success:(void (^)(DataSyncObject *object))success
                                  failure:(void (^)(NSError *error))failure
{
    if ([method isEqualToString:@"DELETE"] || [method isEqualToString:@"PUT"]) {
        ...
        return ^(AFHTTPRequestOperation *operation, id responseObject) {
            if (responseObject) {
                ...
                NSError *error = [NSError errorWithDomain:kDataSyncServicesErrorDomain code:DataSyncServicesEmptyResponseData userInfo:userInfo];
                FAIL_AND_RETURN(error);
            }
        };
                                  ...
                                  failure:(void (^)(NSError *error))failure
{
    return ^(AFHTTPRequestOperation *operation, NSError *error){
        if ([self isUnauthorizedAccessError:error]) {
            [[DataSyncSignIn sharedInstance] authenticateWithInteractiveOption:NO success:^(AFOAuthCredential *credential) {
